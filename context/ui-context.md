...
Tailwind utility names map to these variables. Use `bg-base`, `bg-surface`, `text-copy-primary`, `text-copy-muted`l `border-surface-border`, `text-brand`, `bg-accent-dim`, etc.
...
Both fonts are loaded via `next/font/google` and applied as CSS variables on the html element. The base `body` uses Geist Sans with `antialised`.
...
8 define color pairs. Each pair specifies a dark node fill and a vivid contrasting text color tuned for readability on the dark canvas. Defined in `types/canvas.ts` as `NODE_COLORS`.
