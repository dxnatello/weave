...
- Respect the system boundaries define in `architecture-context.md`.
...
- Add `"use client"` only when the components needs browser interactivity, hooks or real-time state.
...
- Maintain the border radius scale: `rounded-xl` for small elements, `rounded-2xl` for cards and `rounded-3xl` for modals.
...
- `app/api/` - router handlers for auth, triggering and persistence.
